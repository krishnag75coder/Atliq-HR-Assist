...
class LeaveManager:
    ...
    def apply_leave(self, req: LeaveApplyRequest) -> str:
        ...
        # generate request_id
        hist = self.employee_leaves[employee_id]["history"]
        request_id = max([r.get("request_id", 0) for r in hist]) + 1 if hist else 1
        ...
        for d in req.leave_dates:
            ...
            self.employee_leaves[employee_id]["history"].append(history_entry)
